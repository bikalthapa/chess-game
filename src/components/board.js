...
class Moves{
	...
	movePcsTo(pos) {
		let dest_id = "x"+pos.y+"_y"+pos.x;
		let src_id = "x"+this.position.y+"_y"+this.position.x;

		this.bord[pos.x][pos.y] = this.bord[this.position.x][this.position.y];
		document.getElementById(dest_id).innerHTML = this.bord[pos.x][pos.y].pcs;
		document.getElementById(src_id).innerHTML = '';
		document.getElementById(dest_id).style.color = color(this.bord[pos.x][pos.y].typ);
	...
	rookPossibleMoves(plyr, rangeLimit = false){
		let temp;
		...
		for(let i = 1; i<(8-this.position.y); i++){
			...
			if(!this.is_empty(temp) || rangeLimit!=false){
				break;
			}
		}
	...
	bishopPossibleMoves(plyr, rangeLimit = false){
		let temp_position = { x: '', y: '' };
		...
		for (let i = 1; i <(8 - this.position.x); i++) {// for the top left diagonal
			...
			if(this.isPosExists(temp_position)){
				this.set_valid_moves(plyr,temp_position);
				if(!this.is_empty(temp_position) || rangeLimit!=false){
						break;
				}
			}
		}
	...
	queenPossibleMoves(plyr){
		this.bishopPossibleMoves(plyr);
		this.rookPossibleMoves(plyr);
	}
	kingPossibleMoves(plyr){
		this.rookPossibleMoves(plyr,true);
		this.bishopPossibleMoves(plyr, true);
	}
}
...
function Board() {
	...
	function identify_pieces(position) {
		...
		if (pcsSelect === true) {
			...
			}else if(inside_box.pcs === pieces.Queen){
				pcsSelect = true;
				queen(position, inside_box.typ);
			}else if(inside_box.pcs === pieces.King){
				pcsSelect = true;
				king(position, inside_box.typ);
			}
		}
	}
	...
	// for queen movement //
	function queen(position, player){
		mov.setInitPos(position.x, position.y);
		mov.possibleMoves = [];
		mov.queenPossibleMoves(player);
		mov.indicateMoves(true);
	}
	// for king movement //
	function king(position, player){
		mov.setInitPos(position.x, position.y);
		mov.possibleMoves = [];
		mov.kingPossibleMoves(player);
		mov.indicateMoves(true);
	}
